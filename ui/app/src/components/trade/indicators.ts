...
/**
 * The overlays a trader expects on a candle chart.
 *
 * Plain functions over the candle array rather than a library: a moving
 * average is four lines, and pulling in a technical-analysis package to get it
 * would add a second opinion about what a "period" means and a second place
 * where the series shape is defined.
 *
 * Every one of these returns points on the chart's own time axis, so they can
 * be handed to a line series unchanged. Warm-up bars — the first `period - 1`,
 * where the average has nothing to average — are omitted rather than emitted
 * as zero, which would draw a cliff up from the bottom of the chart.
 */
