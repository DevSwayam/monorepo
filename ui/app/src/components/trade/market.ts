/**
 * Illustrative market data for the trading screen.
 *
 * Everything here is a mock of the interface, not a claim about any exchange.
 * Prices, sizes and P&L are internally consistent with each other so a reader
 * does not find arithmetic that does not add up, but they are placeholders.
 * Swap the three `…For(address)` builders for a live feed; nothing in the
 * components below knows where the numbers came from.
 *
 * Everything is generated from a seed derived from the token address, so the
 * server and the client render identical geometry. A random walk evaluated
 * twice hydrates mismatched.
 */
