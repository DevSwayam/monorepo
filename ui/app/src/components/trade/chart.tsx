...
import { type Candle, price as fmtPrice, usd } from "./market";
import {
...
export function PriceChart({
  ...
  return (
    <div className={cn("relative h-full w-full", className)}>
      ...
      >
        {studies.map((study) => (
          <div className="absolute left-2" key={study}>
            <button
              className="glass pointer-events-auto flex items-center gap-1.5 rounded-full py-1 pr-2 pl-3 text-kicker text-fg-subtle shadow-card transition-colors duration-micro ease-smooth-out hover:text-foreground"
      ...
      */}
      {withLegend && legend ? (
        <div className="glass pointer-events-none absolute top-2 left-2 z-10 flex flex-wrap items-baseline gap-x-3 gap-y-0.5 rounded-full px-3.5 py-1.5 text-kicker shadow-card">
          {(
            [
              ["O", legend.o],
              ["H", legend.h],
          ...
          <span className="flex items-baseline gap-1">
            <span className="text-fg-subtle">Vol</span>
          ...
          {maValues.map(({ at, period, tone }) => {
            ...
            return (
              <span className="flex items-baseline gap-1" key={period}>
                <span className="text-fg-subtle">EMA {period}</span>
                <span
                  className="figures"
